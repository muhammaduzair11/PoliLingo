...
| `20260929000000_no_passwords.sql`    | Integration | **As built.** A trigger on `auth.users` blanks any password Supabase Auth tries to store, so password sign-in never succeeds (ADR-0030: Google or an email code only). It closes the pre-registration takeover: a stranger signs up someone's email with a password, the owner later confirms it with a code, and the password would have opened it. Test: `tests/db/passwords.test.mjs` |
