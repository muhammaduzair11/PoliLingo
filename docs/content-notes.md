...
Primary seed reference: [Transparent Language phrase resource](https://tplsites.s3.amazonaws.com/resources/PUSas-ENGus/grammar/ADDITIONAL_INFORMATION.htm). Cross-reference: [Omniglot Pashto phrases](https://www.omniglot.com/language/phrases/pashto.php). Adopt Northern/Peshawar-style `kha` and `Pakhto` readings as the course starting point. This is not a claim every source is Northern-specific. Remove source vowel marks for beginner display and normalize obvious typographical variants; keep actual words in Arabic script. The name Sara and country Pakistan are inserted into sourced introduction templates. Transliteration is an accessible approximation, not IPA and not an audio substitute.
...
Words are introduced before practice. Matching uses expression-to-meaning pairs. Assembly uses English meaning tiles, keeping native RTL text intact rather than teaching incorrect visual ordering. Incorrect answers are appended to a review queue and must be answered correctly before completion. First-try accuracy is based on the initial eight exercises. Repeated mismatches in one matching exercise count as one incorrect initial exercise; all pairs still have to be connected.

No generated voice, native-speaker audio, speech-recognition grading, or claims of native-speaker certification appear in this version. Friendly feedback never imposes a lives/paywall limit.
